...
func TestBuiltInCloudControl_NodeRegistration(t *testing.T) {
	...
	resp, err := cloudControl.NodeRegister(ctx, req)
	require.NoError(t, err)
...
func TestBuiltInCloudControl_ClientCreation(t *testing.T) {
	...
	// 创建用户
	user, err := cloudControl.CreateUser(context.Background(), "testuser", "test@example.com")
	require.NoError(t, err)
	require.NotNil(t, user)

	// 创建客户端
	client, err := cloudControl.CreateClient(context.Background(), user.ID, "test-client")
	require.NoError(t, err)
...
func TestBuiltInCloudControl_JWTTokenGeneration(t *testing.T) {
	...
	// 生成JWT令牌
	tokenInfo, err := cloudControl.GenerateJWTToken(context.Background(), client.ID)
	require.NoError(t, err)
	...
	// 验证令牌
	validatedToken, err := cloudControl.ValidateJWTToken(context.Background(), tokenInfo.Token)
	require.NoError(t, err)
...
func TestBuiltInCloudControl_AnonymousCredentials(t *testing.T) {
	...
	// 生成匿名凭据
	client, err := cloudControl.GenerateAnonymousCredentials(context.Background())
	require.NoError(t, err)
	require.NotNil(t, client)
	...
	// 验证匿名客户端存在
	retrievedClient, err := cloudControl.GetAnonymousClient(context.Background(), client.ID)
	require.NoError(t, err)
...
func TestBuiltInCloudControl_SystemStats(t *testing.T) {
	...
	// 获取系统统计信息
	stats, err := cloudControl.GetSystemStats(context.Background())
	require.NoError(t, err)
