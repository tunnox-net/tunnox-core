...
func AuthMiddleware(cloudControl interface{}) gin.HandlerFunc {
	cc, ok := cloudControl.(interface {
		ValidateToken(ctx context.Context, token string) (*struct {
			Success  bool
			Message  string
			UserID   string
			ClientID string
	...
	if !ok {
		panic("cloudControl does not implement ValidateToken")
	}

	return func(c *gin.Context) {
		authHeader := c.GetHeader(constants.HTTPHeaderAuthorization)
		if authHeader == "" {
			SendUnauthorized(c, "Authorization header required", nil)
			c.Abort()
			return
		}

		if len(authHeader) < 7 || authHeader[:7] != "Bearer " {
			SendUnauthorized(c, "Invalid authorization format", nil)
			c.Abort()
			return
		}

		token := authHeader[7:]
		resp, err := cc.ValidateToken(c.Request.Context(), token)
		if err != nil || resp == nil || !resp.Success {
			SendUnauthorized(c, "Invalid or expired token", err)
			c.Abort()
			return
		}
...
func OptionalAuthMiddleware(cloudControl interface{}) gin.HandlerFunc {
	return func(c *gin.Context) {
		authHeader := c.GetHeader(constants.HTTPHeaderAuthorization)
		if authHeader == "" {
			c.Next()
		...
		_ = authHeader[7:] // token variable, will be used when implementing actual validation
