...
func (cm *CleanupManager) RegisterCleanupTask(ctx context.Context, taskType string, interval time.Duration) error {
	...
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypeStorage, "check task exists failed")
	}
	...
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypePermanent, "marshal task failed")
	}

	if err := cm.storage.Set(key, string(data), 0); err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypeStorage, "save task failed")
	}
...
func (cm *CleanupManager) AcquireCleanupTask(ctx context.Context, taskType string) (*CleanupTask, bool, error) {
	...
	if !ok {
		return nil, false, coreErrors.New(coreErrors.ErrorTypePermanent, "storage does not support CAS operations")
	}
	...
	if err != nil {
		return nil, false, coreErrors.Wrap(err, coreErrors.ErrorTypeStorage, "acquire lock failed")
	}
	if !acquired {
...
func (cm *CleanupManager) CompleteCleanupTask(ctx context.Context, taskType string, err error) error {
	...
	data, err := cm.storage.Get(key)
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypeStorage, "get task failed")
	}

	taskData, ok := data.(string)
	if !ok {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "invalid task data type")
	}

	var task CleanupTask
	if err := json.Unmarshal([]byte(taskData), &task); err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypePermanent, "unmarshal task failed")
	}
	...
	dataBytes, err := json.Marshal(task)
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypePermanent, "marshal completed task failed")
	}
	...
	success, err := casStore.CompareAndSwap(key, taskData, string(dataBytes), 0)
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypeStorage, "update completed task failed")
	}

	if !success {
		return coreErrors.New(coreErrors.ErrorTypeTemporary, "task was modified by another process")
	}
