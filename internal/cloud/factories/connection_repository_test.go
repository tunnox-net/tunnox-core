...
func TestBuiltInCloudControl_ConnectionManagement_WithRepository(t *testing.T) {
	t.Run("RegisterConnection_WithRepository", func(t *testing.T) {
		...
		cloudControl := NewBuiltinCloudControlWithStorageAndServices(context.Background(), config, storage)
	...
	t.Run("GetConnections_WithRepository", func(t *testing.T) {
		...
		// 注意：RegisterConnection 会生成新的 ConnID，所以这里传入的 ConnID 会被覆盖
		conn1 := &models.ConnectionInfo{
		...
		for _, conn := range connections {
			sourceIPs[conn.SourceIP] = true
			assert.Equal(t, createdMapping.ID, conn.MappingID)
			assert.NotEmpty(t, conn.ConnID) // ConnID 应该被自动生成
		}
		assert.True(t, sourceIPs["192.168.1.109"])
		assert.True(t, sourceIPs["192.168.1.110"])
	})
