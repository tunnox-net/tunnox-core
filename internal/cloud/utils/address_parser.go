...
func ParseListenAddress(addr string) (string, int, error) {
	...
	if port < 1 || port > 65535 {
		return "", 0, coreerrors.Newf(coreerrors.CodeInvalidParam, "port %d out of range [1, 65535]", port)
	}
...
func ParseTargetAddress(addr string) (string, int, string, error) {
	...
	if err != nil {
		...
		if portNum < 1 || portNum > 65535 {
			return "", 0, "", coreerrors.Newf(coreerrors.CodeInvalidParam, "port %d out of range [1, 65535]", portNum)
		}
