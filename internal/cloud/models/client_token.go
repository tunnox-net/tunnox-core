...
func (t *ClientToken) Validate() error {
	if t.ClientID <= 0 {
		return coreErrors.Newf(coreErrors.ErrorTypePermanent, "invalid client ID: %d", t.ClientID)
	}
	
	if t.JWTToken == "" {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "jwt_token is required")
	}
	
	if t.TokenID == "" {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "token_id is required")
	}
	
	if t.TokenExpiresAt.IsZero() {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "token_expires_at is required")
	}
