...
func (c *ClientConfig) Validate() error {
	if c.ID <= 0 {
		return coreErrors.Newf(coreErrors.ErrorTypePermanent, "invalid client ID: %d", c.ID)
	}

	if c.AuthCode == "" {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "auth code is required")
	}

	if c.Type != ClientTypeRegistered && c.Type != ClientTypeAnonymous {
		return coreErrors.Newf(coreErrors.ErrorTypePermanent, "invalid client type: %s", c.Type)
	}
