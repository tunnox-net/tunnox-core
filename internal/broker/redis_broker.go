...
func NewRedisBroker(parentCtx context.Context, config *RedisBrokerConfig, nodeID string) (*RedisBroker, error) {
	if config == nil {
		return nil, coreErrors.New(coreErrors.ErrorTypePermanent, "redis broker config is required")
	}
...
func (r *RedisBroker) Publish(ctx context.Context, topic string, message []byte) error {
	...
	if err != nil {
		return coreErrors.Wrap(err, coreErrors.ErrorTypePermanent, "failed to marshal message")
	}
...
func (r *RedisBroker) Subscribe(ctx context.Context, topic string) (<-chan *Message, error) {
	...
	if _, exists := r.subscribers[topic]; exists {
		return nil, coreErrors.Newf(coreErrors.ErrorTypePermanent, "already subscribed to topic: %s", topic)
	}
	...
	if err := r.pubsub.Subscribe(r.Ctx(), channel); err != nil {
		...
		close(msgChan)
		return nil, coreErrors.Wrap(err, coreErrors.ErrorTypeNetwork, "failed to subscribe to Redis")
	}
...
func (r *RedisBroker) Unsubscribe(ctx context.Context, topic string) error {
	...
	ch, exists := r.subscribers[topic]
	if !exists {
		return coreErrors.Newf(coreErrors.ErrorTypePermanent, "not subscribed to topic: %s", topic)
	}
...
func (r *RedisBroker) Ping(ctx context.Context) error {
	...
	if r.client == nil {
		return coreErrors.New(coreErrors.ErrorTypePermanent, "redis client is nil")
	}
