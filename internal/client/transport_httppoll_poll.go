...
func (c *HTTPLongPollingConn) pollLoop() {
	corelog.Debugf("HTTP long polling: pollLoop started, clientID=%d, pollURL=%s", c.clientID, c.pollURL)
	defer corelog.Debugf("HTTP long polling: pollLoop exiting, clientID=%d", c.clientID)

	// 检查 context 是否已取消
	if c.Ctx().Err() != nil {
		corelog.Debugf("HTTP long polling: pollLoop context already cancelled: %v", c.Ctx().Err())
		return
	}
	...
	for {
		select {
		case <-c.Ctx().Done():
			corelog.Debugf("HTTP long polling: pollLoop exiting due to context cancellation: %v", c.Ctx().Err())
			return
		default:
		}
		...
		if err != nil {
			corelog.Errorf("HTTP long polling: failed to parse poll URL: %v", err)
		...
		if err != nil {
			corelog.Errorf("HTTP long polling: failed to create poll request: %v", err)
			time.Sleep(httppollRetryInterval)
			continue
		}
		...
		if err != nil {
			corelog.Errorf("HTTP long polling: failed to encode tunnel package: %v", err)
			time.Sleep(httppollRetryInterval)
			continue
		}
		req.Header.Set("X-Tunnel-Package", encodedPkg)

		corelog.Debugf("HTTP long polling: sending poll request, clientID=%d, mappingID=%s, url=%s", c.clientID, c.mappingID, u.String())
		// 发送长轮询请求
		resp, err := c.pollClient.Do(req)
		if err != nil {
			// 如果是 context 取消，直接退出
			if err == context.Canceled || c.Ctx().Err() != nil {
				corelog.Debugf("HTTP long polling: poll request cancelled, exiting")
				return
			}
			corelog.Debugf("HTTP long polling: poll request failed: %v, retrying...", err)
			time.Sleep(httppollRetryInterval)
			continue
		}

		corelog.Debugf("HTTP long polling: poll request succeeded, status=%d", resp.StatusCode)
		...
		if xTunnelPackage != "" {
			corelog.Warnf("HTTP long polling: received X-Tunnel-Package in keepalive response (unexpected), len=%d, requestID=%s. Keepalive requests should not contain control packets.", len(xTunnelPackage), requestID)
		...
		if err := json.NewDecoder(resp.Body).Decode(&pollResp); err != nil {
			resp.Body.Close()
			corelog.Errorf("HTTP long polling: failed to decode poll response: %v", err)
			time.Sleep(httppollRetryInterval)
			continue
		}
		resp.Body.Close()
		...
		if pollResp.Data != "" {
			...
			corelog.Infof("HTTP long polling: received fragment response, groupID=%s, index=%d/%d, size=%d, isFragment=%v, mappingID=%s",
			...
			corelog.Debugf("HTTP long polling: decoding fragment data, Data field len=%d, mappingID=%s",
			...
			if err != nil {
				...
				corelog.Errorf("HTTP long polling: failed to decode fragment data: %v, Data preview=%s", err, pollResp.Data[:previewLen2])
				time.Sleep(httppollRetryInterval)
				continue
			}
			corelog.Debugf("HTTP long polling: decoded fragment data, len=%d, mappingID=%s",
				len(fragmentData), c.mappingID)

			// 如果是分片，需要重组
			if isFragment {
				corelog.Infof("HTTP long polling: adding fragment to reassembler, groupID=%s, index=%d/%d, size=%d, originalSize=%d, mappingID=%s",
				...
				if err != nil {
					corelog.Errorf("HTTP long polling: failed to add fragment: %v, groupID=%s, index=%d, mappingID=%s",
				...
				corelog.Debugf("HTTP long polling: checking if fragment group complete, groupID=%s, receivedCount=%d/%d, mappingID=%s",
					pollResp.FragmentGroupID, group.ReceivedCount, pollResp.TotalFragments, c.mappingID)
				isComplete := group.IsComplete()
				if !isComplete {
					// 分片组不完整，继续等待更多分片
					corelog.Infof("HTTP long polling: fragment %d/%d received, waiting for more fragments, groupID=%s, receivedCount=%d, mappingID=%s",
				...
				if err != nil {
					corelog.Errorf("HTTP long polling: failed to get next complete group: %v, groupID=%s, mappingID=%s",
				...
				if found {
					corelog.Infof("HTTP long polling: GetNextCompleteGroup found next group, groupID=%s, sequenceNumber=%d, mappingID=%s",
						nextGroup.GroupID, nextGroup.SequenceNumber, c.mappingID)
					// 这是下一个应该发送的分片组，重组并发送
					reassembledData, err := nextGroup.Reassemble()
					if err != nil {
						corelog.Errorf("HTTP long polling: failed to reassemble: %v, groupID=%s, mappingID=%s",
				...
				// 添加到分片重组器，以便按序列号顺序处理
				corelog.Infof("HTTP long polling: received single fragment (TotalFragments=1), adding to reassembler for sequence ordering, groupID=%s, sequenceNumber=%d, mappingID=%s",
					pollResp.FragmentGroupID, pollResp.SequenceNumber, c.mappingID)
				...
				if err != nil {
					corelog.Errorf("HTTP long polling: failed to add single fragment: %v, groupID=%s, mappingID=%s",
						err, pollResp.FragmentGroupID, c.mappingID)
					time.Sleep(httppollRetryInterval)
					continue
				}
				...
				nextGroup, found, err := c.fragmentReassembler.GetNextCompleteGroup()
				if err != nil {
					corelog.Errorf("HTTP long polling: failed to get next complete group for single fragment: %v, groupID=%s, mappingID=%s",
						err, pollResp.FragmentGroupID, c.mappingID)
					c.fragmentReassembler.RemoveGroup(pollResp.FragmentGroupID)
					time.Sleep(httppollRetryInterval)
					continue
				...
				if found {
					corelog.Infof("HTTP long polling: GetNextCompleteGroup found next group for single fragment, groupID=%s, sequenceNumber=%d, mappingID=%s",
						nextGroup.GroupID, nextGroup.SequenceNumber, c.mappingID)
					// 这是下一个应该发送的分片组，重组并发送
					reassembledData, err := nextGroup.Reassemble()
					if err != nil {
						corelog.Errorf("HTTP long polling: failed to reassemble single fragment: %v, groupID=%s, mappingID=%s",
							err, nextGroup.GroupID, c.mappingID)
						c.fragmentReassembler.RemoveGroup(nextGroup.GroupID)
						time.Sleep(httppollRetryInterval)
						continue
					...
					// Base64编码重组后的数据
					base64Data := base64.StdEncoding.EncodeToString(reassembledData)
					corelog.Infof("HTTP long polling: reassembled single fragment, groupID=%s, sequenceNumber=%d, originalSize=%d, base64Len=%d, mappingID=%s",
					...
					case <-c.Ctx().Done():
						return
					case c.base64DataChan <- base64Data:
						corelog.Infof("HTTP long polling: sent single fragment to base64DataChan successfully, size=%d, groupID=%s, sequenceNumber=%d, mappingID=%s",
							len(base64Data), nextGroup.GroupID, nextGroup.SequenceNumber, c.mappingID)
					default:
						corelog.Warnf("HTTP long polling: base64DataChan full, dropping single fragment, size=%d, groupID=%s, sequenceNumber=%d, mappingID=%s",
							len(base64Data), nextGroup.GroupID, nextGroup.SequenceNumber, c.mappingID)
					}
					...
					// 这不是下一个应该发送的分片组，等待序列号更小的分片组完成
					corelog.Infof("HTTP long polling: single fragment complete but GetNextCompleteGroup returned not found, groupID=%s, sequenceNumber=%d, waiting for expected sequence, mappingID=%s",
						pollResp.FragmentGroupID, pollResp.SequenceNumber, c.mappingID)
				}
			}
		} else if pollResp.Timeout {
			corelog.Debugf("HTTP long polling: poll request timeout, retrying...")
		}
