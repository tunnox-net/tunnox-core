...
func (m *CommandResponseManager) HandleResponse(pkt *packet.TransferPacket) bool {
	...
	if !pkt.PacketType.IsCommandResp() || pkt.CommandPacket == nil {
		corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FAILED] Reason=not_CommandResp_or_no_CommandPacket, Time=%s",
	...
	if commandID == "" {
		corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FAILED] Reason=CommandID_empty, Time=%s",
			time.Now().Format("15:04:05.000"))
		return false
	}

	corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_START] CommandID=%s, Time=%s",
	...
	if !exists {
		corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FAILED] CommandID=%s, Reason=no_pending_request, Time=%s",
	...
	corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FOUND] CommandID=%s, Time=%s",
		commandID, time.Now().Format("15:04:05.000"))
	...
	if !stillExists {
		corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FAILED] CommandID=%s, Reason=request_unregistered, Time=%s",
			commandID, time.Now().Format("15:04:05.000"))
		return false
	}
	...
	case responseChan <- resp:
		...
		corelog.Debugf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_COMPLETE] CommandID=%s, Success=%v, HandleDuration=%v, Time=%s",
			commandID, resp.Success, handleDuration, time.Now().Format("15:04:05.000"))
		return true
	default:
		corelog.Warnf("[CMD_TRACE] [CLIENT] [RESP_HANDLE_FAILED] CommandID=%s, Reason=channel_full, Time=%s",
			commandID, time.Now().Format("15:04:05.000"))
		corelog.Warnf("Response channel is full for command %s", commandID)
		return false
	}
}
