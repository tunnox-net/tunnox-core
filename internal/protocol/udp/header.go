...
func (h *TUTPHeader) Encode(buf []byte) (int, error) {
	if len(buf) < HeaderLength() {
		return 0, coreErrors.Newf(coreErrors.ErrorTypeProtocol, "buffer too small: need %d bytes, got %d", HeaderLength(), len(buf))
	}
...
func DecodeHeader(buf []byte) (*TUTPHeader, int, error) {
	if len(buf) < HeaderLength() {
		return nil, 0, coreErrors.Newf(coreErrors.ErrorTypePermanent, "buffer too small: need %d bytes, got %d", HeaderLength(), len(buf))
	}
	...
	if h.Version != TUTPVersion {
		return nil, 0, coreErrors.Newf(coreErrors.ErrorTypeProtocol, "invalid version: expected %d, got %d", TUTPVersion, h.Version)
	}
	...
	// 校验分片数量
	if h.FragCount < 1 {
		return nil, 0, coreErrors.Newf(coreErrors.ErrorTypeProtocol, "invalid frag count: %d", h.FragCount)
	}
