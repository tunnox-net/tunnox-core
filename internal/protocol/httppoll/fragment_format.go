...
func CreateFragmentResponse(data []byte, fragmentIndex int, fragmentSize int, totalFragments int, originalSize int, groupID string) *FragmentResponse {
	// 获取分片数据
...
func CreateCompleteResponse(data []byte) *FragmentResponse {
...
func SplitDataIntoFragments(data []byte) ([]*FragmentResponse, error) {
	dataSize := len(data)
	fragmentSize, totalFragments := CalculateFragments(dataSize)

	if totalFragments == 1 {
		// 不分片，直接返回完整数据
		return []*FragmentResponse{CreateCompleteResponse(data)}, nil
	}
	...
	for i := 0; i < totalFragments; i++ {
		fragmentResp := CreateFragmentResponse(data, i, fragmentSize, totalFragments, dataSize, groupID)
		if fragmentResp == nil {
