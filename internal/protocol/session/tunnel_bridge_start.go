...
func (b *TunnelBridge) Start() error {
	...
	select {
	case <-b.ready:
		corelog.Infof("TunnelBridge[%s]: target connection established, starting bridge", b.tunnelID)
	...
	if b.sourceForwarder == nil {
		...
		corelog.Infof("TunnelBridge[%s]: recreating sourceForwarder, sourceConn=%v, sourceStream=%v", b.tunnelID, b.sourceConn != nil, b.sourceStream != nil)
	...
	if b.targetForwarder == nil {
		...
		corelog.Infof("TunnelBridge[%s]: recreating targetForwarder, targetConn=%v, targetStream=%v", b.tunnelID, b.targetConn != nil, b.targetStream != nil)
	...
	if b.sourceForwarder == nil || b.targetForwarder == nil {
		corelog.Infof("TunnelBridge[%s]: connection does not support data forwarding (sourceForwarder=%v, targetForwarder=%v), bridge only manages connection lifecycle",
	...
	corelog.Infof("TunnelBridge[%s]: bridge started, transparent forwarding (no compression/encryption on server)", b.tunnelID)
	...
	go func() {
		for {
			...
			if sourceForwarder == nil {
				corelog.Warnf("TunnelBridge[%s]: sourceForwarder is nil, waiting...", b.tunnelID)
				time.Sleep(100 * time.Millisecond)
				continue
			}
			corelog.Infof("TunnelBridge[%s]: starting source->target copy", b.tunnelID)
			written := b.copyWithControl(b.targetForwarder, sourceForwarder, "source->target", &b.bytesSent)
			corelog.Infof("TunnelBridge[%s]: source->target copy finished, %d bytes", b.tunnelID, written)
			...
			if newSourceForwarder == nil {
				corelog.Infof("TunnelBridge[%s]: sourceForwarder is nil, exiting", b.tunnelID)
				break
			}
			if newSourceForwarder == sourceForwarder {
				corelog.Infof("TunnelBridge[%s]: sourceForwarder unchanged, exiting", b.tunnelID)
				break
			}
			corelog.Infof("TunnelBridge[%s]: sourceForwarder updated, continuing with new connection", b.tunnelID)
		}
	}()
	...
	go func() {
		...
		corelog.Infof("TunnelBridge[%s]: target->source finished, %d bytes", b.tunnelID, written)
	}()
