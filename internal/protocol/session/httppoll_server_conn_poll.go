...
func (c *ServerHTTPLongPollingConn) PushData(base64Data string) error {
	...
	if closed {
		corelog.Warnf("HTTP long polling: [PUSHDATA] connection closed, clientID=%d", c.clientID)
	...
	select {
	case <-c.Ctx().Done():
		corelog.Warnf("HTTP long polling: [PUSHDATA] context canceled, clientID=%d", c.clientID)
		return c.Ctx().Err()
	case c.base64PushDataChan <- base64Data:
		corelog.Debugf("HTTP long polling: [PUSHDATA] Base64 data pushed successfully, clientID=%d", c.clientID)
		return nil
	default:
		corelog.Errorf("HTTP long polling: [PUSHDATA] base64PushDataChan full, clientID=%d", c.clientID)
...
func (c *ServerHTTPLongPollingConn) pollDataScheduler() {
	corelog.Infof("HTTP long polling: [POLLDATA_SCHEDULER] started, clientID=%d", c.clientID)
	...
	for {
		select {
		case <-c.Ctx().Done():
			corelog.Debugf("HTTP long polling: [POLLDATA_SCHEDULER] context canceled, clientID=%d", c.clientID)
			return
		case <-ticker.C:
...
func (c *ServerHTTPLongPollingConn) PollData(ctx context.Context) (string, error) {
	...
	select {
	case <-ctx.Done():
		corelog.Debugf("HTTP long polling: [POLLDATA] context canceled, clientID=%d", c.clientID)
		return "", ctx.Err()
	case <-c.Ctx().Done():
		corelog.Debugf("HTTP long polling: [POLLDATA] connection context canceled, clientID=%d", c.clientID)
		return "", c.Ctx().Err()
	case <-c.pollWaitChan:
	...
	case data, ok := <-c.pollDataChan:
		if !ok {
			corelog.Debugf("HTTP long polling: [POLLDATA] channel closed, clientID=%d", c.clientID)
			return "", io.EOF
		}
		corelog.Infof("HTTP long polling: [POLLDATA] received %d bytes from channel, encoding to Base64, clientID=%d",
			len(data), c.clientID)
