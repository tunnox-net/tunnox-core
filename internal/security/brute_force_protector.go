...
func (p *BruteForceProtector) RecordFailure(ip string) bool {
	...
	// 永久封禁判断
	if record.TotalCount >= p.config.PermanentBanAt {
		p.banIP(ip, 0, fmt.Sprintf("累计失败 %d 次", record.TotalCount))
		utils.Warnf("BruteForce: IP %s permanently banned after %d total failures", ip, record.TotalCount)
		return true
	}
	
	// 临时封禁判断
	if recentFailures >= p.config.MaxFailures {
		p.banIP(ip, p.config.BanDuration, fmt.Sprintf("时间窗口内失败 %d 次", recentFailures))
	...
	utils.Debugf("BruteForce: IP %s failed %d/%d times (total: %d)", 
		ip, recentFailures, p.config.MaxFailures, record.TotalCount)
...
func (p *BruteForceProtector) BanIP(ip string, duration time.Duration, reason string) {
	...
	p.banIP(ip, duration, reason)
}

// banIP 内部封禁方法（调用者需持有锁）
func (p *BruteForceProtector) banIP(ip string, duration time.Duration, reason string) {
	now := time.Now()
