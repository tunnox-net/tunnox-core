...
func (s *ManagementAPIServer) handleHTTPPush(w http.ResponseWriter, r *http.Request) {
	corelog.Debugf("HTTP long polling: [HANDLE_PUSH] received Push request, method=%s, contentLength=%d", r.Method, r.ContentLength)

	// 1. 获取并解码 X-Tunnel-Package（必须）
	packageHeader := r.Header.Get("X-Tunnel-Package")
	if packageHeader == "" {
		corelog.Errorf("HTTP long polling: [HANDLE_PUSH] missing X-Tunnel-Package header")
		s.respondError(w, http.StatusBadRequest, "missing X-Tunnel-Package header")
		return
	}
	corelog.Debugf("HTTP long polling: [HANDLE_PUSH] X-Tunnel-Package len=%d", len(packageHeader))
			...
			if err := json.Unmarshal(bodyBytes, &pushReq); err == nil {
				...
				if pushReq.Data != "" {
					...
					if isFragment {
						...
						if err != nil {
							corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to decode fragment data: %v, connID=%s", err, connID)
						...
						if err != nil {
							corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to add fragment: %v, connID=%s", err, connID)
							s.respondError(w, http.StatusBadRequest, "Failed to add fragment")
							return
						}

						corelog.Debugf("HTTP long polling: [HANDLE_PUSH] added fragment %d/%d, groupID=%s, connID=%s",
						...
						if err != nil {
							corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to reassemble fragments: %v, connID=%s", err, connID)
						...
						if isComplete {
							...
							corelog.Debugf("HTTP long polling: [HANDLE_PUSH] reassembled %d bytes from %d fragments, groupID=%s, connID=%s",
							...
							if err := streamProcessor.PushData(base64Data); err != nil {
								corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to push reassembled data: %v, connID=%s", err, connID)
								streamProcessor.GetFragmentReassembler().RemoveGroup(pushReq.FragmentGroupID)
							...
							corelog.Debugf("HTTP long polling: [HANDLE_PUSH] fragment %d/%d received, waiting for more fragments or already reassembled, groupID=%s, connID=%s",
								pushReq.FragmentIndex, pushReq.TotalFragments, pushReq.FragmentGroupID, connID)
						}
					} else {
						// 完整数据，直接推送
						if err := streamProcessor.PushData(pushReq.Data); err != nil {
							corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to push data: %v, connID=%s", err, connID)
							s.respondError(w, http.StatusServiceUnavailable, "Connection closed")
							return
						}
						corelog.Debugf("HTTP long polling: [HANDLE_PUSH] pushed data successfully, dataLen=%d, connID=%s", len(pushReq.Data), connID)
					}
				} else {
					corelog.Debugf("HTTP long polling: [HANDLE_PUSH] body parsed but data field is empty, connID=%s", connID)
				}
			} else {
				corelog.Debugf("HTTP long polling: [HANDLE_PUSH] failed to parse JSON body: %v, bodyLen=%d, connID=%s", err, len(bodyBytes), connID)
			}
		} else if err != nil {
			corelog.Debugf("HTTP long polling: [HANDLE_PUSH] failed to read body: %v, connID=%s", err, connID)
		}
	}
	...
	if pkg.Type != "" {
		corelog.Debugf("HTTP long polling: [HANDLE_PUSH] processing control package, type=%s, connID=%s", pkg.Type, connID)
	...
	if responsePkg != nil {
		...
		if err == nil {
			...
			corelog.Debugf("HTTP long polling: [HANDLE_PUSH] set X-Tunnel-Package header, len=%d, connID=%s", len(encodedPkg), connID)
		} else {
			corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to encode response package: %v, connID=%s", err, connID)
		}
	}

	// 9. 返回响应
	corelog.Debugf("HTTP long polling: [HANDLE_PUSH] preparing response, connID=%s", connID)
	...
	corelog.Debugf("HTTP long polling: [HANDLE_PUSH] writing ACK response, connID=%s", connID)
	if err := json.NewEncoder(w).Encode(resp); err != nil {
		corelog.Errorf("HTTP long polling: [HANDLE_PUSH] failed to write response: %v, connID=%s", err, connID)
		return
	}
	corelog.Debugf("HTTP long polling: [HANDLE_PUSH] response written successfully, connID=%s", connID)
}
